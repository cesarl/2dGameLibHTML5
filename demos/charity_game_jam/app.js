...
var Canvas, Collection, Event, Node, Pubsub, Reader, Square, Texture, Tile, Update,
  __slice = [].slice;
...
Pubsub = (function() {
  ...
  Pubsub.prototype.subscribe = function(topic, callback, weight) {
    ...
    elem.weight = weight;
    elem.topic = topic;
    _ref = this.cache[topic];
    for (i = _i = 0, _len = _ref.length; _i < _len; i = ++_i) {
      obj = _ref[i];
      if (obj.weight > elem.weight) {
        this.cache[topic].splice(i, 0, elem);
        return elem;
      }
    }
...
Event = (function() {
  ...
  function Event(domElem, en) {
    ...
    this.domElem.addEventListener("mousedown", function(e) {
      ...
      _this.mouseup = false;
      return DD.publish("mousedown");
    });
    this.domElem.addEventListener("mouseup", function(e) {
      _this.mousedown = false;
      _this.mouseup = true;
      return DD.publish("mouseup");
    });
    this.domElem.addEventListener("keydown", function(e) {
      ...
      switch (e.keyCode) {
        case 38 || 90:
          _this.up = true;
          return DD.publish("up");
        case 40 || 83:
          _this.down = true;
          return DD.publish("down");
        case 37 || 81:
          _this.left = true;
          return DD.publish("left");
        case 39 || 68:
          _this.right = true;
          return DD.publish("right");
        case 32:
          _this.space = true;
          return DD.publish("space");
        case 27:
          _this.esc = true;
          return DD.publish("esc");
        default:
          _this.other = true;
          return DD.publish("other");
      }
    });
...
Canvas = (function() {
  ...
  function Canvas(canvasId, w, h) {
    ...
    this.h = h != null ? h : 600;
    this.type = "Canvas";
    this.subHandler = DD.subscribe("raf:frame:tick", (function() {
      return _this.clear();
    }), 0);
  ...
  Canvas.prototype.fullScreen = function(active) {
    var _this = this;
    if (active == null) {
      active = true;
    }
    if (active) {
      ...
      this._resizeFullScreenFn = function() {
        return _this.canvas.style.height = window.innerHeight + 'px';
      };
      window.addEventListener("resize", this._resizeFullScreenFn, false);
    } else {
      this.canvas.style.height = this.canvas.h + 'px';
      this.canvas.style.width = this.canvas.w + 'px';
...
Update = (function() {
  ...
  function Update(tickRate, expiration, callback, callbackEnd, index) {
  ...
  Update.prototype.start = function(index) {
    var _this = this;
    this.active = true;
    return this.subHandler = DD.subscribe("raf:frame:tick", (function() {
      return _this.step();
    }), index);
  };
...
Node = (function() {
  ...
  Node.prototype.drawTile = function(canvas, zindex) {
    var _this = this;
    this._drawTile = new Update(1, -1, (function() {
      return _this.drawImgSquare(canvas, _this.tile, _this.square);
    }));
    return this;
  };

  Node.prototype.drawTexture = function(canvas, zindex) {
    var _this = this;
    this._drawTexture = new Update(1, -1, (function() {
      return _this.drawImgColSquare(canvas, _this.texture, _this.square);
    }), null, zindex);
    return this;
  };

  Node.prototype.drawImgColSquare = function(canvas, texture, square) {
    var col, i;
    i = 0;
    col = texture.col;
    while (col[i]) {
  ...
  Node.prototype.drawImgSquare = function(canvas, tile, square) {
...
(function(document) {
  var COL_col, COL_ctx, COL_evt, COL_img, COL_node, COL_spt, cycle, facade, filterSelector, filterSelectorsCollection, gImg, gSpt, gsCol, gsCtx, gsEvt, gsNode, imgLoader, parseSelectors, _cycleRunning, _global, _name, _oldDD, _pubsub;
  _name = "2dGameLib";
  _global = this;
  _oldDD = _global.DD;
  _cycleRunning = false;
  ...
  COL_col = {};
  COL_spt = {};
  parseSelectors = function(sel, params) {
    sel = sel.split(" ").join("");
    sel = sel.split(",");
    if (sel.len > 1) {
  ...
  filterSelectorsCollection = function(sel, params) {
    console.log("filterSelectorsCollection", sel, sel.length);
    return sel;
  };
  filterSelector = function(sel, params) {
    var l;
    l = sel[0];
    sel = sel.slice(1, sel.length);
    if (l === '!') {
      return gsCtx(sel, params);
    } else if (l === '^') {
      return gsEvt(sel, params);
    } else if (l === '.') {
      return gsNode(sel, params);
    } else if (l === '@') {
      return gImg(sel);
    } else if (l === '#') {
      return gsCol(sel, params);
    } else if (l === '|') {
      return gSpt(sel);
    }
  };
  gsCtx = function(name, params) {
    if (!COL_ctx[name]) {
  ...
  cycle = function() {
    return DD.publish("raf:frame:tick");
  };
  imgLoader = function() {
    var args, callback, img, src,
      _this = this;
    src = arguments[0], callback = arguments[1], args = 3 <= arguments.length ? __slice.call(arguments, 2) : [];
    img = new Image();
    if (src) {
      img.src = src;
    } else {
  ...
  facade = function(selectors, params) {
    var p;
    p = Array.prototype.slice.call(arguments);
    p.splice(0, 1);
    if (this !== document) {
      return facade.selector(selectors, p);
    }
    return facade;
  };
  facade.loadImages = function(collection, callback) {
    return facade.loadImage(collection, 0, callback);
  };
  facade.loadImage = function(collection, index, callback) {
    var _this = this;
    return COL_img[collection[index].name] = imgLoader(collection[index].src, function() {
      if (collection[++index]) {
        return facade.loadImage(collection, index, callback);
      } else {
        return callback();
      }
    });
  };
  facade.stopCycle = function(isStats) {
    if (isStats == null) {
      isStats = false;
    }
    _cycleRunning = false;
  ...
  facade.startCycle = function(isStats) {
    var anim, stats,
      _this = this;
    if (isStats == null) {
      isStats = false;
  ...
  facade.subscribe = function() {
    return _pubsub.subscribe.apply(_pubsub, arguments);
  };
  facade.publish = function() {
    return _pubsub.publish.apply(_pubsub, arguments);
  };
  facade.unsubscribe = function() {
    return _pubsub.unsubscribe.apply(_pubsub, arguments);
  };
  facade.selector = function(selectors, params) {
    if (selectors && typeof selectors === "string") {
      return parseSelectors(selectors, params);
    }
  };
  facade.kill = function(selectors) {
    var col;
    if (selectors && typeof selectors === "string") {
      col = parseSelectors(selectors);
      col.killAll();
      if (col.type === "Canvas") {
        return delete COL_ctx[name];
      } else if (col.type === "Event") {
        return delete COL_evt[name];
      } else if (col.type === "Node") {
        return delete COL_node[name];
      } else if (col.type === "Image") {
        return delete COL_img[name];
      } else if (col.type === "Collection") {
        return delete COL_col[name];
      }
    }
  };
  facade.newNode = function() {
    return new Node();
  };
  facade.buildSprite = function(list, callback, index) {
    if (index == null) {
      index = 0;
    }
    COL_spt[list[index].name] = list[index];
    if (list[++index]) {
      return facade.buildSprite(list, callback, index);
    } else if (callback) {
      return callback();
    }
  };
  return _global.DD = _global[_name] = facade;
})(document);
